...
test('calcFineGrainedStepDivision() - throws error if stepDivision total ends up higher than 100', () => {
  const errMsg = '>100 divisions is too many for this to work well'
  expect(() => calcFineGrainedStepDivision({ firstStep: 1, lastStep: 3, stepDivision: 51 })).toThrow(errMsg)
})
...
test('calcFineGrainedStepDivision() - if >50 and <=100 then return 10', () => {
  expect(calcFineGrainedStepDivision({ firstStep: 1, lastStep: 1, stepDivision: 51 })).toEqual(10)
})
...
test('calcFineGrainedStepDivision() - if >10 and <=20 then return 5', () => {
  expect(calcFineGrainedStepDivision({ firstStep: 1, lastStep: 2, stepDivision: 10 })).toEqual(2)
})
test('calcFineGrainedStepDivision() - if >10 and <=20 then return 5', () => {
  expect(calcFineGrainedStepDivision({ firstStep: 1, lastStep: 1, stepDivision: 11 })).toEqual(2)
})
